...
D0 - Skill ID. See the list of constants for what skill IDs correspond to what.
D1 - Coin Price. You need to spend this many coins to buy this skill. For upgrades that are not brought yet, this price is displayed below the node.
D2 - Modifier icon. Tile offset for the little modifier icon that is displayed in the bottom left corner of the node (eg. the + icon for Strong Water)
D4-7 - Unlock nodes. The nodes defined by FFC ID are unlocked when you purchase this node. They are split into two halves to squeeze two args out of each of these. Left corresponds to the number on the left side of the decimal point while right corresponds to the number on the right side of the decimal point. The right side *MUST* always have 4 digits - just add 0s in front of your number until you have 4 digits if you plan on using it.
	Setting these args also draws lines between these nodes to communicate to the player what these nodes unlock. If a particuar line is 1-way (the connection is linked to by only one node), an arrow is drawn instead of a solid line.
...
enum
{
	...
	SKILL_CHEAPFIREMINUS,	//Minor reduction in Fire MP consumption
	SKILL_CHEAPFIRE,		//Moderate Reduction in Fire MP consumption
	SKILL_CHEAPFIREPLUS,	//Large reduction in Fire MP consumption
...
namespace SkillTree
{
	enum
	{
		...
		NODE_SKILL,			//Int: The skill the node gives. D0 on its corresponding FFC.
		NODE_PRICE,			//Int: The coin price of the node. D1 on its corresponding FFC.
		NODE_MODICON,		//Int: The modifier icon on the FFC. D2 on its corresponding FFC.
	...
	void Operate(int map, int screen)
	{
		...
		for(int fcmb = 1; fcmb <= 32; fcmb ++)
		{
			if(TreeScreen->NumFFCs[fcmb]) //Valid FFC
			{
				...
				node[(fcmb-1)*NODE_END+NODE_SKILL] = TreeScreen->GetFFCInitD(fcmb, 0);
				node[(fcmb-1)*NODE_END+NODE_PRICE] = TreeScreen->GetFFCInitD(fcmb, 1);
				node[(fcmb-1)*NODE_END+NODE_MODICON] = TreeScreen->GetFFCInitD(fcmb, 2);
